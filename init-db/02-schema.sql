...
JOIN Encuentros e ON p.id_encuentro = e.id_encuentro
;
REM INSERTING into ENCUENTROS_ADMIN.AMISTADES
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.APORTES
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.BOLSILLOS
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.ENCUENTROS
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.GASTOS
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.ITEMS_PRESUPUESTO
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.MENSAJES
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.PARTICIPANTES_ENCUENTRO
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.PRESUPUESTOS
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.RECORDATORIOS
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.RELACIONES_AMISTADES
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.SOLICITUDES_AMISTAD
SET DEFINE OFF;
REM INSERTING into ENCUENTROS_ADMIN.USUARIOS
SET DEFINE OFF;
Insert into ENCUENTROS_ADMIN.USUARIOS (ID_USUARIO,NOMBRE,APELLIDO,EMAIL,CONTRASENA,IMAGEN_PERFIL,FECHA_REGISTRO) values (21,'Tomi','Ramirez','tomi@gmail.com','$2b$10$3Se841fyUDf.uD4tugBnVOQA1infMxWCyBCsBbjdtAipcQ6HxfuQW',null,to_timestamp('06-NOV-25 02.08.06.094000000 AM','DD-MON-RR HH.MI.SSXFF AM'));
REM INSERTING into ENCUENTROS_ADMIN."migrations"
SET DEFINE OFF;
...
set define off;
  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ENCUENTROS_ADMIN"."ACEPTAR_SOLICITUD_AMISTAD" (
    p_id_relacion_amistad IN NUMBER,   -- ID de la relación de amistad
...
BEGIN
    -- 1️⃣ Obtener datos de la relación: quién envió la solicitud y su estado
    SELECT id_usuario, estado
    INTO v_usuario_origen, v_estado
    FROM RELACIONES_AMISTADES
    WHERE id_relacion_amistad = p_id_relacion_amistad;

    -- 2️⃣ Si el estado ya está en 'aceptada', lanzar error
    IF v_estado = 'aceptada' THEN
        RAISE_APPLICATION_ERROR(-20002, 'La solicitud de amistad ya está aceptada.');
    END IF;

    -- 3️⃣ Obtener el usuario destino (a quién se le envió la solicitud)
    SELECT usuario_destino
    INTO v_usuario_destino
    FROM SOLICITUDES_AMISTAD
    WHERE id_relacion_amistad = p_id_relacion_amistad;

    -- 4️⃣ Validar que el usuario que intenta aceptar es efectivamente el destinatario
    ...
    -- 5️⃣ Actualizar el estado de la relación a 'aceptada' y registrar la fecha
    UPDATE RELACIONES_AMISTADES
    SET estado = 'aceptada',
        fecha_aceptacion_amistad = CURRENT_TIMESTAMP
    WHERE id_relacion_amistad = p_id_relacion_amistad;

    -- 6️⃣ Insertar en la tabla AMISTADES
    INSERT INTO AMISTADES (
        ...
        v_usuario_origen,   -- quien envió la solicitud
        v_usuario_destino,  -- quien la aceptó
        CURRENT_TIMESTAMP
    );
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20001, 'No se encontró la solicitud o la relación de amistad.');
    WHEN OTHERS THEN
        ROLLBACK;
        RAISE;
...
BEGIN
    -- 1️⃣ Verificar que el bolsillo pertenezca al encuentro indicado
    SELECT id_encuentro
    ...
    -- 2️⃣ Verificar que el usuario pertenezca al encuentro
    SELECT COUNT(*)
    ...
    -- 4️⃣ Actualizar el saldo del bolsillo
    UPDATE Bolsillos
...
BEGIN
    ...
    -- 5️⃣ Actualizar saldo del bolsillo
    UPDATE Bolsillos
    SET saldo_actual = saldo_actual - p_monto
    WHERE id_bolsillo = p_id_bolsillo;
...
BEGIN
    -- 1️⃣ Verificar que el presupuesto exista y obtener su id_encuentro
    SELECT id_encuentro
    INTO v_id_encuentro_presupuesto
    FROM Presupuestos
    WHERE id_presupuesto = p_id_presupuesto;

    -- 2️⃣ Validar que el id_encuentro proporcionado corresponda al del presupuesto
    ...
    -- 3️⃣ Insertar el nuevo ítem en la tabla Items_Presupuesto
    ...
    -- 4️⃣ Actualizar el total del presupuesto
    UPDATE Presupuestos
...
BEGIN
    -- 1️⃣ Verificar si el rol no permitido
    ...
    -- 2️⃣ Verificar si el usuario ya está agregado en el encuentro
    SELECT COUNT(*)
    INTO v_count_participante
    FROM Participantes_Encuentro
    ...
    IF v_count_participante > 0 THEN
        RAISE_APPLICATION_ERROR(-20001, 'El usuario ya está agregado en este encuentro.');
    END IF;

    -- 3️⃣ Obtener el ID del creador del encuentro
    SELECT id_creador
    INTO v_id_creador
    FROM Encuentros
    WHERE id_encuentro = p_id_encuentro;

    -- 4️⃣ Verificar si el usuario es amigo del creador (en ambas direcciones)
    SELECT COUNT(*)
    ...
    -- 5️⃣ Insertar el nuevo participante
...
BEGIN
    ...
    -- 2️⃣ Registrar al creador como organizador
    INSERT INTO Participantes_Encuentro (
        id_encuentro,
        id_usuario,
        rol
    ...
    -- 3️⃣ Crear el presupuesto base del encuentro
    ...
    -- 4️⃣ Crear el bolsillo asociado al presupuesto
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20001, 'No se encontró información al crear el encuentro.');
...
BEGIN
    -- 1️⃣ Verificar si el usuario destino ya le envió una solicitud al origen (solicitud cruzada)
    SELECT COUNT(*)
    ...
    -- 2️⃣ Verificar si ya existe una solicitud pendiente en el mismo sentido
    SELECT COUNT(*)
    ...
    -- 3️⃣ Insertar registro en RELACIONES_AMISTADES
    ...
    -- 4️⃣ Obtener el id_usuario (usuario_origen) desde la tabla RELACIONES_AMISTADES recién creada
    SELECT id_usuario
      INTO v_usuario_origen
      FROM RELACIONES_AMISTADES
     WHERE id_relacion_amistad = v_id_relacion_amistad;

    -- 5️⃣ Insertar el registro en SOLICITUDES_AMISTAD
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(' ESTADÍSTICAS FINANCIERAS DE ENCUENTROS ');
    DBMS_OUTPUT.PUT_LINE('--------------------------------------------------------------');
    DBMS_OUTPUT.PUT_LINE('ID | TÍTULO | ESTADO | PRESUPUESTO | GASTOS | BALANCE | % EJEC');
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20010, 'Error al generar las estadísticas: ' || SQLERRM);
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE "ENCUENTROS_ADMIN"."PKG_CENTRAL" AS
    ...
    -- Agregar un ítem al presupuesto
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "ENCUENTROS_ADMIN"."PKG_CENTRAL" AS
    ...
    BEGIN
        -- Llama al procedimiento existente que maneja todo el flujo de creación de encuentros
        crear_encuentro(
